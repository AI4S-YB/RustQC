...
pub fn compute(cov: &TssCov) -> Vec<NfrRow> {
    ...
    assert!(
        flank >= 200,
        "TssCov flank must be >=200 for NFRscore (got {})",
        flank
    );
    ...
    for buf in &cov.buffers {
        let n1: f64 = buf[flank - 200..flank - 50]
            .iter()
            .map(|&v| v as f64)
            .sum::<f64>()
            / N as f64;
        let nf: f64 = buf[flank - 50..flank + 50]
            .iter()
            .map(|&v| v as f64)
            .sum::<f64>()
            / F as f64;
        let n2: f64 = buf[flank + 50..flank + 200]
            .iter()
            .map(|&v| v as f64)
            .sum::<f64>()
            / N as f64;
